...
<div class="flex flex-col md:flex-row bg-white p-6 md:py-7 md:px-8 rounded-[0.625rem] text-13 {currentStatus !== "" ? borderStyles : ""}" data-sveltekit-preload-data="hover">
     ...
     <a href="/suggestions/{suggestion?._id}" class="block flex-1 md:pr-[1.5625rem]">
          ...
          <h2 class="font-bold text-[#3A4374] md:text-lg leading-3">{suggestion?.title}</h2>
          <p class="text-[#647196] md:text-base my-2 md:mb-4">{suggestion?.description}</p>
          <span class="hidden md:inline-block bg-[#F2F4FF] rounded-[0.625rem] px-4 py-1 font-semibold text-[#4661E6]">{suggestion?.tag}</span>
     </a>
